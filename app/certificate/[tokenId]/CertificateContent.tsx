...
export function CertificateContent({ tokenId: tokenIdStr }: { tokenId: string }) {
  ...
  useEffect(() => {
    async function fetchCertificate() {
      try {
        ...
        const [certData, valid] = await Promise.all([
          client.readContract({
  ...
  const getVerificationURL = () => {
    if (typeof window !== 'undefined') {
      return `${window.location.origin}/verify?tokenId=${tokenIdStr}`;
    }
    return `/verify?tokenId=${tokenIdStr}`;
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Token ID: {tokenIdStr}</p>
          <p className="mt-1 text-sm text-gray-500">Contract: {getContractAddress()}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              No certificate exists with token ID {tokenIdStr}
            </p>
            {error && (
              <div className="text-sm text-red-600 mb-4 text-left">
                <p className="font-semibold">Error:</p>
                <p className="break-all">{String(error)}</p>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className="font-medium text-gray-900">{tokenIdStr}</span>
                    </div>
                    <div>
            ...
            <div className="mt-8 pt-6 border-t flex flex-wrap gap-4">
              <Button
                onClick={() => window.open(`https://sepolia.etherscan.io/token/${getContractAddress()}?a=${tokenIdStr}`, '_blank')}
                variant="outline"
              >
                View on Etherscan
              </Button>
